...
#include "mmu_armv8a.h"
 ...
 static uint64_t us_lvl3_table[512][512] __attribute__((aligned(4096)));
...
#define TT_S1_PAGE_PXN		(uint64_t) (0x0000000000000001 << 53)
#define TT_S1_TABLE_PXN		(uint64_t) 0x800000000000000
#define TT_S1_PAGE_EL0_RW	(uint64_t) (0x0000000000000001 << 6)
...
void mmu_setup_tables()
{
	...
	/* Userspace */
	/* [0]: Virt: 00 0000,0000 - 0x3FFF,FFFF */
...
void mmu_init()
{
	...
	_set_tcr_el1(
		...
		(0x1<<23)   /* EPD1 = 0b01. Enable table walks from TTBR1 */
